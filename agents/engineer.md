...
task would benefit from a skill outside this default set (5 preloaded + `agent-skills` +
...
No skill here is optional context — all five are preloaded at startup and always active, applied
directly rather than invoked. Beyond these five, `engineer` also holds a scoped `Skill` grant
covering the full `agent-skills` and `superpowers` plugins (see the `tools:` line above for the
exact allowlist), invoked on demand when one of them fits better than the five preloaded skills.

**Default skill set = the 5 preloaded skills + `agent-skills:*` + `superpowers:*`. Nothing else.**
...
- Never invokes a skill outside the default set (5 preloaded + `agent-skills:*` + `superpowers:*`)
